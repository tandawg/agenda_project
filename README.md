В проекте agenda_project представлено веб-приложение для управления задачами с использованием базы данных SQLite. Приложение реализовано на языке Go и предоставляет API для взаимодействия, а также статические веб-страницы для работы с задачами через браузер.
...
1. Перейдите в директорию с тестами: cd tests
2. Проверьте параметры в settings.go:
    ServerPort = "7540"
    TestDBPath = "../scheduler.db"
    FullNextDate = false
    Search = true
3. Запустите тесты: go test ./tests
